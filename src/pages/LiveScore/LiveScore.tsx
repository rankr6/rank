...
const MatchList = () => {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        // Fetch sports after user preferences are retrieved
      } catch (error) {
  ...
  const filteredSports = sports.filter((sport) => sportsData.includes(Number(sport.id)));
    ...
    <>
      ...
      <div className="overflow-x-auto">
        <div className="flex space-x-4">
          {matchScores
  .filter((match) =>
    filteredSports.length === 0 || filteredSports.some(sport => sport.name === match.sportName)
  )
  .map((match) => (
    <div
      key={match.id}
